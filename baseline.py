if __name__ == "__main__":
    ...
    # Step 2: Generate predictions from the baseline model
    baseline_predictions = baseline_model.predict(val_dataloader, return_y=True)

    # Step 3: Extract the outputs and targets
    baseline_outputs = baseline_predictions.output  # Predicted values
    baseline_targets = baseline_predictions.y       # Ground truth values

    print(f"Shape of baseline predictions: {baseline_outputs.shape}")
    print(f"Shape of baseline targets: {baseline_targets['decoder_target'].shape}")

    # Step 4: Compute Mean Absolute Error (MAE)
    mae_metric = MAE()
    baseline_mae = mae_metric(baseline_outputs, baseline_targets)
    print(f"Baseline Model MAE: {baseline_mae.item()}")
