if __name__ == "__main__":
    ...
    training_set = TimeSeriesDataSet(
        ...
        target_normalizer=TorchNormalizer(method="standard"),  # Global normalization
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='val_loss',
        dirpath='./ckp',
        filename='tft-test_5-{epoch:02d}-{train_loss:.2f}',
        save_top_k=3,
        mode='min',
        save_last=True,
    ...
    trainer = pl.Trainer(
        ...
        logger=logger,
        log_every_n_steps=200,  # Log metrics every 50 steps
        accumulate_grad_batches=4,
    )


    # NEXT:
    # Try 0.5 dropout, remove accumulate_grad_batches
